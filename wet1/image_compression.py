...
def find_best_d_by_quality():
    ...
    fig = plt.figure()
    fig.suptitle("AVG Quality, Error and CR by d values")
    plt.subplot(3, 1, 1)
    ...
    for d in [1, 10, 20]:
        pca_obj = PCA(d)
        pca_obj.fit(patches_dataset)
        image_before = normalized_images[0].reshape(62, 47)
        image_after = compress_restore_image(image_before, pca_obj)
        plt.figure()
        ax = plt.subplot(1, 2, 1)
        ax.set_title("Original Image")
        plt.imshow(image_before)
        ax = plt.subplot(1, 2, 2)
        ax.set_title("Compressed Image with d={}".format(d))
